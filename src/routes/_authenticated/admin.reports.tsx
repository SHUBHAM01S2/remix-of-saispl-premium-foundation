...
import { deleteReport, listReports, slugifyClient } from "@/lib/reports-admin.functions";
...
function ReportsRoute() {
  ...
  if (location.pathname.replace(/\/$/, "") !== "/admin/reports") return <Outlet />;
  return <ReportsPage />;
}
    ...
    <div className="min-h-[80vh] bg-background px-4 py-16">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-foreground">Client Reports</h1>
            <p className="mt-1 text-sm text-muted-foreground">Monthly performance reports per client.</p>
          </div>
          <Link
        ...
        <div className="mt-6 overflow-hidden rounded-2xl border border-border/60 bg-card shadow-sm">
          {isLoading ? (
            ...
            <p className="px-6 py-8 text-sm text-muted-foreground">No reports yet.</p>
          ) : (
                  ...
                  {data.map((r) => {
                    ...
                    return (
                      <tr key={r.id}>
                        ...
                        <td className="px-4 py-3 text-muted-foreground">{Number(r.uptime_percentage).toFixed(2)}%</td>
                        <td className="px-4 py-3 text-muted-foreground">{r.site_visitors.toLocaleString()}</td>
                        <td className="px-4 py-3 text-muted-foreground">{r.leads_from_forms}</td>
                        <td className="px-4 py-3 text-muted-foreground">{r.whatsapp_taps}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button
                              ...
                              onClick={() => {
                                if (confirm(`Delete report for ${r.client_name} (${new Date(r.month).toLocaleDateString(undefined, { month: "short", year: "numeric" })})?`)) {
                                  del.mutate(r.id);
                                }
                              }}
