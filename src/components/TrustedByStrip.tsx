            ...
            <ul className="flex w-max animate-marquee-rtl items-center gap-4 sm:gap-10 group-hover:[animation-play-state:paused]">
              {loop.map((logo, i) => (
                <li key={`${logo.name}-${i}`} className="shrink-0">
                  <div
                    className="flex h-20 w-[150px] items-center justify-center rounded-xl bg-[#1b4166] px-4 py-3 shadow-sm ring-1 ring-black/5 sm:h-32 sm:w-[280px] sm:px-6 sm:py-4"
                    title={logo.name}
                  >
                    <img
                      src={logo.src}
