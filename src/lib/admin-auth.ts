...
export async function assertAnyAdmin(ctx: Ctx): Promise<AdminRow> {
...
export async function assertSuperAdmin(ctx: Ctx): Promise<AdminRow> {
  const row = await fetchAdmin(ctx);
...
export async function assertContentEditor(ctx: Ctx): Promise<AdminRow> {
  const row = await fetchAdmin(ctx);
